...
const publishArgs = args.filter(arg => 
...
async function main() {
  ...
  try {
    ...
    const versionCmd = `node ${versionScript} ${versionType}${prereleaseTag ? ` ${prereleaseTag}` : ''} ${shouldTag ? '--tag' : ''} ${dryRun ? '--dry-run' : ''} ${flags.includes('--commit') ? '--commit' : ''}`;
